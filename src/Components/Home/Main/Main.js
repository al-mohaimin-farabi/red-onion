...
const Main = () => {
  ...
  const getFoodById = (id) => {
    const data = Object.keys(restaurentData)
      .map((key) => restaurentData[key])
      .map((foodType) => foodType.find((food) => food.id === id))
    ...
    <main>
      ...
      {showFoodContainer && (
        <FoodContainer
          ...
          hideAll={hideAll}
          setShowChekout={setShowChekout}
        />
      )}
      {showSingleFood && Object.keys(filteredFood).length > 0 && (
        <SingleFoodContainer
          food={filteredFood}
          addToCart={addToCart}
          goBack={goBack}
