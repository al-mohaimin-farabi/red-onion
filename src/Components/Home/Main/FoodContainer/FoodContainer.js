    ...
    return (
        <>
            <div className="food-container">
                {
                    filteredData && filteredData.map(food => <Food key={food.title} food={food} getFoodById={getFoodById} />)
                }
            </div>
            <button onClick={() => { hideAll(); setShowChekout(true) }} disabled={context.cart.length < 1} className="checkout-btn">Checkout Your Food</button>
        </>
    );
};
